...
func (o *User) UserWorkspaces(mods ...qm.QueryMod) userWorkspaceQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`user_workspaces`.`user_id`=?", o.ID),
	)
...
func (userL) LoadUserDevices(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`user_devices.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadUserWorkspaces(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`user_workspaces.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.User appropriately.
func (o *User) AddUserWorkspaces(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserWorkspace) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
