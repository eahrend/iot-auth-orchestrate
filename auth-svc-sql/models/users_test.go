...
func testUserToManyUserDevices(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserDevices(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyUserWorkspaces(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserWorkspaces(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpUserDevices(t *testing.T) {
	var err error
...
func testUserToManyAddOpUserWorkspaces(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `Username`: `varchar`, `Password`: `varchar`}
	_           = bytes.MinRead
)
